...
@Component({
  ...
  template: `
    <div class="space-y-6 bg-sky-100 p-6 rounded-lg">
      <div class="bg-white rounded-lg shadow-sm p-4">
        <h1 class="text-2xl font-bold text-slate-800">Monthly Invoice</h1>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white rounded-lg shadow-md p-6">
          ...
          <p class="mt-2 text-3xl font-bold text-blue-600">{{ totalInvoiceValue() | currency:'':'symbol':'1.2-2' }}</p>
        </div>
        <div class="bg-white rounded-lg shadow-md p-6">
          <p class="text-sm font-medium text-slate-500">Total Collected Value</p>
          <p class="mt-2 text-3xl font-bold text-green-600">{{ totalCollectedValue() | currency:'':'symbol':'1.2-2' }}</p>
        </div>
        <div class="bg-white rounded-lg shadow-md p-6">
          <p class="text-sm font-medium text-slate-500">Total Outstanding Value</p>
          <p class="mt-2 text-3xl font-bold text-orange-500">{{ totalOutstandingValue() | currency:'':'symbol':'1.2-2' }}</p>
        </div>
      </div>
      ...
      <div class="bg-white p-6 rounded-lg shadow-md">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="relative">
              <label for="division" class="absolute -top-2 left-2 inline-block bg-white px-1 text-xs font-medium text-gray-500">Select Division</label>
              <select #divisionSelect (change)="selectedDivision.set(divisionSelect.value)" id="division" class="block w-full rounded-md border-0 py-2.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
             ...
             <div class="relative">
              <label for="status" class="absolute -top-2 left-2 inline-block bg-white px-1 text-xs font-medium text-gray-500">Select Status</label>
              <select #statusSelect (change)="selectedStatus.set(statusSelect.value)" id="status" class="block w-full rounded-md border-0 py-2.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
...
export class MonthlyInvoiceComponent {
  ...
  // Data
  private allInvoices = this.invoiceService.monthlyInvoices;
  ...
  totalOutstandingValue = computed(() => this.filteredInvoices().reduce((sum, inv) => sum + inv.outstandingValue, 0));
